...
async function main() {
  ...
  const result = await new Promise((resolve,reject) => {
    ...
    ws.send(JSON.stringify({id,type:"message",channel:ch,message:{id,command:"export_node_as_image",params:{nodeId:"54772:569",format:"PNG",scale:2}}}));
  });
  const buf = Buffer.from(result.imageData, "base64");
  fs.writeFileSync(path.resolve(__dirname, "../belo-home-v2.png"), buf);
  console.log("Saved belo-home-v2.png (" + buf.length + " bytes)");
