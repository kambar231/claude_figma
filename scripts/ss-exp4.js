...
async function main() {
  ...
  const result = await new Promise((resolve,reject) => {
    ...
    ws.send(JSON.stringify({id,type:"message",channel:ch,message:{id,command:"export_node_as_image",params:{nodeId:"54779:1135",format:"PNG",scale:2}}}));
  });
  fs.writeFileSync(path.resolve(__dirname, "../experiments/exp4-result.png"), Buffer.from(result.imageData, "base64"));
