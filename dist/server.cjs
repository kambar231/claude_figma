...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to modify"),
    gradientType: import_zod.z.enum(["GRADIENT_RADIAL", "GRADIENT_LINEAR"]).describe("Type of gradient"),
    gradientStops: import_zod.z.array(
      import_zod.z.object({
        r: import_zod.z.number().min(0).max(1).describe("Red (0-1)"),
        g: import_zod.z.number().min(0).max(1).describe("Green (0-1)"),
        b: import_zod.z.number().min(0).max(1).describe("Blue (0-1)"),
        a: import_zod.z.number().min(0).max(1).describe("Alpha (0-1)"),
        position: import_zod.z.number().min(0).max(1).describe("Stop position (0-1)")
      })
    ).describe("Gradient color stops"),
    gradientHandlePositions: import_zod.z.array(import_zod.z.object({ x: import_zod.z.number(), y: import_zod.z.number() })).length(3).optional().describe(
      "Three handle positions [center, x-axis end, y-axis end] in 0-1 space. Defaults to radial centered fill."
    ),
    opacity: import_zod.z.number().min(0).max(1).optional().describe("Overall opacity (0-1), default 1")
  },
...
server.tool(
  ...
  "Set effects (drop shadow, inner shadow, blur) on a node in Figma",
  {
    ...
    effects: import_zod.z.array(import_zod.z.object({
      ...
      offset: import_zod.z.object({ x: import_zod.z.number(), y: import_zod.z.number() }).optional().describe("Offset for shadow effects"),
      radius: import_zod.z.number().optional().describe("Blur radius"),
      spread: import_zod.z.number().optional().describe("Spread for shadow effects"),
      visible: import_zod.z.boolean().optional().describe("Whether effect is visible")
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to modify"),
    opacity: import_zod.z.number().min(0).max(1).describe("Opacity value (0=transparent, 1=opaque)")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to fill with an image"),
    imageData: import_zod.z.string().describe("Base64-encoded PNG or JPG image data"),
    scaleMode: import_zod.z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("How the image scales"),
    opacity: import_zod.z.number().min(0).max(1).optional().describe("Image opacity")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to modify"),
    blendMode: import_zod.z.enum(["NORMAL", "DARKEN", "MULTIPLY", "COLOR_BURN", "LIGHTEN", "SCREEN", "COLOR_DODGE", "OVERLAY", "SOFT_LIGHT", "HARD_LIGHT", "DIFFERENCE", "EXCLUSION", "HUE", "SATURATION", "COLOR", "LUMINOSITY"]).describe("Blend mode")
  },
...
server.tool(
  ...
  "Create an ellipse/circle in Figma",
  {
    x: import_zod.z.number().describe("X position"),
    y: import_zod.z.number().describe("Y position"),
    width: import_zod.z.number().positive().describe("Width"),
    height: import_zod.z.number().positive().describe("Height"),
...
server.tool(
  ...
  "Create a vector shape from SVG path data in Figma (for icons, custom shapes, etc.)",
  {
    ...
    width: import_zod.z.number().positive().optional().describe("Width to resize vector to"),
    height: import_zod.z.number().positive().optional().describe("Height to resize vector to"),
    name: import_zod.z.string().optional().describe("Name for the vector"),
    parentId: import_zod.z.string().optional().describe("Parent node ID"),
    paths: import_zod.z.array(import_zod.z.object({
      data: import_zod.z.string().describe("SVG path data string (e.g. 'M 0 0 L 10 10 L 20 0 Z')"),
      windingRule: import_zod.z.enum(["NONZERO", "EVENODD"]).optional().describe("Winding rule for the path")
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to set fills on"),
    fills: import_zod.z.array(import_zod.z.object({
      type: import_zod.z.enum(["SOLID", "GRADIENT_LINEAR", "GRADIENT_RADIAL"]).describe("Fill type"),
      color: import_zod.z.object({
        r: import_zod.z.number().min(0).max(1),
        g: import_zod.z.number().min(0).max(1),
        b: import_zod.z.number().min(0).max(1)
      }).optional().describe("Color for SOLID fills (r,g,b in 0-1 range)"),
      opacity: import_zod.z.number().min(0).max(1).optional().describe("Fill opacity"),
      visible: import_zod.z.boolean().optional().describe("Whether fill is visible"),
      blendMode: import_zod.z.string().optional().describe("Blend mode (e.g. NORMAL, MULTIPLY)"),
      gradientHandlePositions: import_zod.z.array(import_zod.z.object({
        x: import_zod.z.number(),
        y: import_zod.z.number()
      })).optional().describe("Three gradient handle positions [start, end, width]"),
      gradientStops: import_zod.z.array(import_zod.z.object({
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to apply effects to"),
    effects: import_zod.z.array(import_zod.z.object({
      type: import_zod.z.enum(["DROP_SHADOW", "INNER_SHADOW", "LAYER_BLUR", "BACKGROUND_BLUR"]).describe("Effect type"),
      color: import_zod.z.object({
        r: import_zod.z.number().min(0).max(1),
      ...
      radius: import_zod.z.number().optional().describe("Blur radius"),
      spread: import_zod.z.number().optional().describe("Shadow spread (for shadow effects)"),
      visible: import_zod.z.boolean().optional().describe("Whether effect is visible"),
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(2).describe("Array of node IDs to group (minimum 2)"),
    name: import_zod.z.string().optional().describe("Name for the group")
  },
  async ({ nodeIds, name }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to flatten")
  },
  async ({ nodeId }) => {
    try {
...
server.tool(
  ...
  "Add effects (drop shadow, inner shadow, layer blur, background blur) to a node in Figma",
  {
    ...
    effects: import_zod.z.array(import_zod.z.object({
      ...
      offset: import_zod.z.object({
        x: import_zod.z.number().describe("X offset"),
        y: import_zod.z.number().describe("Y offset")
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to set opacity on"),
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to fill with an image"),
    imageData: import_zod.z.string().describe("Base64 encoded image data (PNG or JPG)"),
    scaleMode: import_zod.z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("How the image scales within the node"),
    opacity: import_zod.z.number().min(0).max(1).optional().describe("Opacity of the image fill (0-1)")
  },
  async ({ nodeId, imageData, scaleMode, opacity }) => {
    try {
      const result = await sendCommandToFigma("set_image_fill", { nodeId, imageData, scaleMode, opacity });
...
server.tool(
  ...
  "Set the blend mode of a node in Figma",
  {
    nodeId: import_zod.z.string().describe("The ID of the node to set blend mode on"),
...
server.tool(
  ...
  "Create a new ellipse (circle or oval) in Figma",
  {
    x: import_zod.z.number().describe("X position"),
    y: import_zod.z.number().describe("Y position"),
    width: import_zod.z.number().describe("Width of the ellipse"),
    height: import_zod.z.number().describe("Height of the ellipse"),
    name: import_zod.z.string().optional().describe("Optional name for the ellipse"),
    parentId: import_zod.z.string().optional().describe("Optional parent node ID to append the ellipse to")
  },
  async ({ x, y, width, height, name, parentId }) => {
    try {
