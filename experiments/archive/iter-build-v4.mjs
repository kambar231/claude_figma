/**
 * Iteration 4: Chat v1.3
 * Fixes: tighter message spacing to prevent overflow, all messages visible
 */
...
function nextId() { return `v4-${++reqCounter}`; }
...
async function build() {
  const screenX=21500, vName="Chat v1.3";
  console.log(`\n=== ${vName} at (${screenX}, 0) ===\n`);
  ...
  ],[{x:0.5,y:0.30},{x:1.15,y:0.30},{x:0.5,y:1.0}]);
  await sleep(100);
  ...
  let curY=mmY+mmM.height+12; // reduced from 18 to 12
  let lastS=null;

  for(const m of msgs){
    const sc=m.sender!==lastS;
    curY += sc ? 14 : 4; // reduced from 20/6 to 14/4
    ...
    if(sc){
      ...
      curY+=sM.height+1; // reduced from +2 to +1
    }
    ...
    curY+=mn.height+1; // reduced from +2 to +1
  ...
  try {
    ...
    if(img?.imageData){
      ...
      const p=path.join(__dirname,"iteration-4.png");
