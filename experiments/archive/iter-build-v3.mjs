/**
 * Iteration 3: Chat v1.2
 * Fixes: tighter gradient, phone btn position, spacing, ball glow
 */
...
function nextId() { return `v3-${++reqCounter}`; }
  ...
  return new Promise((resolve, reject) => {
    http.get(CHANNELS_URL, (res) => {
      ...
      res.on("end", () => {
        ...
        const s = Object.entries(ch).sort((a,b) => b[1]-a[1]);
        s.length > 0 ? resolve(s[0][0]) : reject(new Error("No channel"));
      });
    }).on("error", reject);
  });
}
      ...
      try {
        ...
        if (d.type==="broadcast" && d.message) {
          ...
          if (m.id && pending.has(m.id)) {
            const {resolve:r,timer} = pending.get(m.id);
            clearTimeout(timer); pending.delete(m.id);
            r(m.error ? {error:m.error} : (m.result||m));
          }
        }
      } catch(_){}
    });
...
const createFrame = o => cmd("create_frame", {x:o.x??0,y:o.y??0,width:o.width??100,height:o.height??100,name:o.name??"Frame",parentId:o.parentId,fillColor:o.fillColor});
const createRect = o => cmd("create_rectangle", {x:o.x??0,y:o.y??0,width:o.width??100,height:o.height??100,name:o.name??"Rect",parentId:o.parentId});
const createEllipse = o => cmd("create_ellipse", {x:o.x??0,y:o.y??0,width:o.width??50,height:o.height??50,name:o.name??"Ell",parentId:o.parentId});
...
async function measureText(opts) {
  ...
  if (opts.maxWidth && w > opts.maxWidth) {
    await resizeNode(r.id, opts.maxWidth, h);
    await sleep(120);
...
async function build() {
  ...
  // BG gradient - tighter center, darker edges
  const bg = await createRect({x:0,y:0,width:W,height:H,name:"bg",parentId:screenId});
  await setGrad(bg.id, "GRADIENT_RADIAL", [
    {r:0.275,g:0.110,b:0.340,a:0.95,position:0},
    {r:0.220,g:0.090,b:0.280,a:0.8,position:0.2},
    {r:0.165,g:0.070,b:0.220,a:0.55,position:0.4},
    {r:0.100,g:0.045,b:0.160,a:0.3,position:0.6},
    {r:0.060,g:0.025,b:0.110,a:0.1,position:0.8},
  ...
  // STATUS BAR
  const isl = await createRect({x:(W-126)/2,y:11,width:126,height:37,name:"isl",parentId:screenId});
  await setFillC(isl.id, C.black, 1); await setRadius(isl.id, 20);

  await createText({text:"2:32",fontSize:16,fontWeight:600,fontColor:C.white,parentId:screenId,x:28,y:15,name:"time"});

  for (let i=0;i<4;i++) {
    const bH=4+i*2;
    const b = await createRect({x:W-100+i*5,y:18+(10-bH),width:3,height:bH,name:`s${i}`,parentId:screenId});
    await setFillC(b.id, C.white, 1); await setRadius(b.id, 0.5);
  }
  const wX=W-74;
  ...
  // Phone btn - moved to x=90
  const pSz=42, pX=90, pY=btnCY-pSz/2;
  await glassCircle(pX,pY,pSz,"phone");
  const phCX=pX+pSz/2, phCY=pY+pSz/2;
  const phX=phCX-9, phY=phCY-9;
  let t;
  ...
  const glSz=130;
  const gR=await createRect({x:ballX-(glSz-ballSz)/2,y:ballY-(glSz-ballSz)/2,width:glSz,height:glSz,name:"bg-gl",parentId:screenId});
  ...
  await createText({text:"belo",fontSize:26,fontWeight:400,fontFamily:FONT_LOGO,fontColor:{r:1,g:0.98,b:0.95},parentId:screenId,
    x:ballX+(ballSz-bM.width)/2, y:ballY+(ballSz-bM.height)/2-2, name:"ball-t"});
  ...
  await createText({text:"8 members",fontSize:13,fontWeight:400,fontColor:C.textMuted,parentId:screenId,
    x:(W-mmM.width)/2,y:mmY,name:"mem"});
  ...
  for (const m of msgs) {
    ...
    const tsM=await measureText({text:m.time,fontSize:11,fontWeight:400,fontColor:C.textMuted});
    await createText({text:m.time,fontSize:11,fontWeight:400,fontColor:C.textMuted,parentId:screenId,
  ...
  const sp=await createRect({x:0,y:inY-1,width:W,height:1,name:"sp",parentId:screenId});
  await setFillC(sp.id,C.white,0.05);
  ...
  // "belo" placeholder - more centered
  ...
  await createText({text:"belo",fontSize:20,fontWeight:400,fontFamily:FONT_LOGO,fontColor:C.textMuted,parentId:screenId,
    x:(W-hM.width)/2-10, y:inY+(aH-hM.height)/2, name:"inp-belo"});
  ...
  const mW=10,mH=16;
  const miIX=miX+(miSz-mW)/2, miIY=miY+10;
  ...
  try {
    ...
    if (img?.imageData) {
      ...
      const p=path.join(__dirname,"iteration-3.png");
